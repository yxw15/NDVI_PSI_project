...
get_species_palette <- function() {
  cat("Defining species list and color palette...\n")
...
generate_plot <- function(data, variable, palette, theme) {
  ...
  print(plot)
  output_file <- paste0("results_moran/AllSpecies_", variable, "_moran_I_vs_distance.png")
...
save_results <- function(data, variable) {
  file <- paste0("results_moran/data/AllSpecies_", variable, "_moran_data.csv")
...
main <- function() {
  ...
  variables <- c("Quantiles", "soil_water_potential", "transpiration_deficit")
  ...
  cat("All processing completed for all species and variables.\n")
}
