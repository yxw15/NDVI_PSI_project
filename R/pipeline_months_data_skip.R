# -----------------------------------------------------------------------------
# Title: NDVI, PSI, and TDiff Multi-Depth Processing Script (with Skip Existing)
# -----------------------------------------------------------------------------
...
process_species <- function(species_name, species_paths, NDVI_file, month_day, depth_val, month_name) {
  ...
  species_paths$output_dir <- file.path(sprintf("results_monthly_%d", depth_val), month_name, species_name)
  ...
  if (file.exists(save_path)) {
    message(sprintf("   - Skipping %s (already processed)", species_name))
    load(save_path)
    return(species_df)
  }
...
process_month <- function(month_name, depth_val) {
  ...
  if (file.exists(out_file)) {
    message(sprintf("   - Skipping month %s (already processed)", month_name))
    load(out_file)
    return(month_df)
  }
...
for (d in depths) {
  ...
  if (file.exists(raw_file)) {
    message(sprintf("   - Skipping combined file for depth %d (already exists)", d))
    load(raw_file)
  } else {
