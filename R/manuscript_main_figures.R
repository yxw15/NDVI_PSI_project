...
plot_time_series_and_correlation_combined(combined,
                                          "results/Figures/depth150/time_series_Quantiles_PSI_TDiff_species_depth150.png")

# depth 100 #
load("results/Data/AllSpecies_AllMonths_depth100.RData")
plot_time_series_and_correlation_combined(combined,
                                          "results/Figures/depth100/time_series_Quantiles_PSI_TDiff_species_depth100.png")

# depth 50 #
load("results/Data/AllSpecies_AllMonths_depth50.RData")
plot_time_series_and_correlation_combined(combined,
                                          "results/Figures/depth50/time_series_Quantiles_PSI_TDiff_species_depth50.png")
...
# depth 150 #
load("results/Data/AllSpecies_AllMonths_depth150.RData")
combined <- combined %>% filter(Quantiles > 0)
combined <- combined %>% filter(month %in% c("July", "August"))

plot_NDVI_PSI_exp_linear_slope_coeff <- function(data, combined_coef_fig, output_figure) {
  ...
  combined_coeff <- p_coeff_linear + p_coeff_exp +
    plot_layout(widths = c(0.8, 1.2), guides = "collect") &
    theme(
...
plot_NDVI_PSI_exp_linear_slope_coeff(combined, 
                                     "results/Figures/depth150/NDVI_Q_PSIbin_exp_linear_coeff.png",
                                     "results/Figures/depth150/NDVI_Q_PSIbin_exp_linear_slope.png")
...
plot_NDVI_PSI_exp_linear_slope_coeff(combined, 
                                     "results/Figures/depth100/NDVI_Q_PSIbin_exp_coeff_depth100.png",
                                     "results/Figures/depth100/NDVI_Q_PSIbin_exp_slope_depth100.png")
...
plot_NDVI_PSI_exp_linear_slope_coeff(combined, 
                                    "results/Figures/depth50/NDVI_Q_PSIbin_exp_coeff_depth50.png",
                                    "results/Figures/depth50/NDVI_Q_PSIbin_exp_slope_depth50.png")
...
plot_NDVI_TDiff_exp_linear_slope_coeff <- function(data, combined_coef_fig, output_figure) {
  ...
  aic_df <- map_dfr(species_all, function(sp) {
    ...
    aic_e <- if(!is.null(exp_mod)) AIC(exp_mod) else NA
    tibble(species = sp, AIC_linear = aic_l, AIC_exponential = aic_e)
  })
  message("AIC comparison for each species:")
  print(aic_df)
  ...
  p_coeff_exp <- ggplot(exp_coef_df, aes(x = species, y = Value, fill = species)) +
    ...
    labs(title = "exponential models",
         ...
         x = NULL, y = "coefficient value", caption = "(b)") +
    theme_minimal() +
    theme(
      axis.text.x = element_text(angle = 0, hjust = 0.5),
      plot.background = element_rect(fill = "white", color = "white"),
  ...
  combined_coeff <- p_coeff_linear + p_coeff_exp +
    plot_layout(widths=c(0.8,1.2), guides="collect")
  print(combined_coeff)
  ggsave(filename = combined_coef_fig, plot = combined_coeff, width = 14, height = 8, dpi = 300)
...
plot_NDVI_TDiff_exp_linear_slope_coeff(combined,
                                       "results/Figures/depth150/NDVI_Q_TDiffbin_exp_linear_coeff.png",
                                       "results/Figures/depth150/NDVI_Q_TDiffbin_exp_linear_slope.png")

plot_NDVI_TDiff_exp_slope_coeff <- function(data, combined_coef_fig, output_figure) {
  # Process data and remove missing rows
  data <- NDVI_TDiffbin(data) %>% na.omit()
...
plot_NDVI_TDiff_exp_slope_coeff(combined,
                                "results/Figures/depth150/NDVI_Q_TDiffbin_exp_coeff.png",
                                "results/Figures/depth150/NDVI_Q_TDiffbin_exp_slope.png")
