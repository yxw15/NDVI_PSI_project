# NDVI-PSI-TDiff Multi-Depth Processing Script
# ------------------------------------------------------------
# Description:
# This script processes NDVI, PSI (soil water potential), and TDiff (transpiration deficit) data
# across multiple soil depths and months for a set of tree species.
# It performs the following steps:
#  1. Loads required R packages and sets global parameters (date range, depths, months, species).
#  2. Defines helper functions to:
#     - Read PSI and TDiff from NetCDF into data frames.
#     - Save yearly PSI and TDiff rasters at specified depths and dates.
#     - Combine NDVI, PSI, and TDiff rasters masked by species.
#     - Orchestrate per-species, per-month, per-depth processing and aggregate results.
#  3. Iterates through configured depths, months (April–August), species (Beech, Oak, Spruce, Pine),
#     and years (2003–2024), saving intermediate GeoTIFFs and final RData files.
# Inputs:
#  - NetCDF files containing PSImean and TDiffsum metrics for species.
#  - NDVI quantile NetCDF files for specific day-of-year per month.
#  - Species-specific mask TIFF rasters (MODIS-based).
# Outputs:
#  - GeoTIFF rasters of PSI and TDiff for each year, depth, and month.
#  - Masked NDVI, PSI, and TDiff rasters per species and year.
#  - Combined data frames saved as .RData per species-month-depth and aggregated per depth.
...
transfer_psi_to_df <- function(nc_file) {
  ...
  dimnames(psi) <- list(x = x_vals, y = y_vals, depth = depth_vals, time = as.character(dates))
  
  df <- as.data.frame.table(psi, responseName = "soil_water_potential", stringsAsFactors = FALSE) %>%
    mutate(
...
save_psi_raster <- function(df, month_day, depth_val, out_dir) {
  ...
  sub <- df %>%
    filter(format(as.Date(time), "%m-%d") == month_day, depth == depth_val)
...
transfer_tdiff_to_df <- function(nc_file) {
  ...
  dates <- as.Date(dates_start) + time
  dimnames(tdiff) <- list(x = x_vals, y = y_vals, time = as.character(dates))
  
  df <- as.data.frame.table(tdiff, responseName = "transpiration_deficit", stringsAsFactors = FALSE) %>%
    mutate(
      x = as.numeric(as.character(x)),
      y = as.numeric(as.character(y)),
      time = as.Date(time)
...
process_month <- function(month_name, depth_val) {
  ...
  dfs <- lapply(names(species_config), process_species, month_name = month_name, depth_val = depth_val)
  month_df <- bind_rows(dfs) %>% mutate(month = month_name)
  
  out_dir <- file.path("results", "Data", sprintf("depth_%d", depth_val))
  save(month_df, file = file.path(out_dir, sprintf("AllSpecies_%s_depth%d.RData", month_name, depth_val)))
...
for (d in depths) {
  ...
  save(combined, file = file.path(base_dir, sprintf("AllSpecies_AllMonths_depth%d.RData", d)))
