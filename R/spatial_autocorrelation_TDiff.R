...
TDiff <- data$transpiration_deficit
...
for (i in seq_along(dist_seq)) {
  ...
  if (sum(unlist(nb)) == 0) {
    ...
    test <- moran.test(TDiff, lw, zero.policy = TRUE)
  ...
  # Save the individual result as a CSV file with the naming format "TDiff_spatial_[distance].csv"
  file_name <- sprintf("results_spatial/TDiff_spatial_%.1f.csv", d)
...
p <- ggplot(results_spatial, aes(x = distance, y = Moran_I)) +
  ...
  labs(x = "Distance Threshold",
       y = "Moran's I",
       title = "Moran's I vs. Distance for TDiff") +
  theme_minimal()
...
# ===============================================================
# Save the combined results data frame as a CSV file named "TDiff_spatial_distance.csv"
write.csv(results_spatial, file = "results_spatial/TDiff_spatial_distance.csv", row.names = FALSE)

# Save the plot as a PNG file
ggsave("results_spatial/results_TDiff_spatial_plot.png", plot = p, width = 8, height = 6)
