...
# =========================
# Core builder: species = axes, lines = MODELS
# metric = "AIC" or "R2"
# =========================
make_radar_models_by_metric <- function(long_tbl, model_levels, metric = c("AIC","R2"),
                                        panel_title_expr, r_label = 1.55, show_legend = FALSE,
                                        draw_lines = FALSE, point_size = 3.8) {
  ...
  # >>> FIX: convert model to character in mutate(), then select
  radar_df <- scaled |>
  ...
  # ---- Axis labels (species) placed on the circle, with per-species offsets ----
  axis_labels <- species_levels
  n_vars <- length(axis_labels)
  angles <- seq(0, 2*pi, length.out = n_vars + 1)[-(n_vars + 1)]
  
  # Customize how far each species label sits from the circle
  # (make Beech & Pine farther than Oak & Spruce)
  ...
  axis_df <- data.frame(
    ...
    mutate(
      x = r * sin(ang),
      y = r * cos(ang),
      # Side-aware justification: right=left-aligned, left=right-aligned
      hjust = case_when(
        sin(ang) >  1e-8 ~ 0,   # right side
        sin(ang) < -1e-8 ~ 1,   # left side
        TRUE             ~ 0.5  # top/bottom
      ),
      # Top/bottom vertical tweaks
      vjust = case_when(
  ...
  expand_r <- max(r_each) + 0.18  # a bit beyond the farthest label
  
  # Colors for present models, in model_levels order
  present_models <- as.character(radar_df$group)
  cols <- unname(model_palette[present_models])
  
  p <- ggradar(
    radar_df,
    axis.labels               = rep("", n_vars),  # custom labels below
    group.colours             = cols,
    ...
    axis.label.size           = 0,
    group.line.width          = if (draw_lines) 1.2 else 0,  # << turn off lines
    ...
    theme(
      ...
      plot.background  = element_rect(fill = "white", colour = NA),
      panel.background = element_rect(fill = "white", colour = NA),
      plot.margin = margin(18, 26, 28, 26)
    ) +
  ...
  if (show_legend) {
    p <- p + guides(colour = guide_legend(title = NULL, nrow = 1, byrow = TRUE))
  }
  p
}
...
add_under_tag <- function(plot, tag_text) {
  tag <- ggdraw() + draw_label(tag_text, x = 0.5, y = 0.95, hjust = 0.5, vjust = 1)
...
# Row 1 (AIC only) — models as lines
pA_AIC <- make_radar_models_by_metric(panelA_long, c("linear","exponential"), "AIC", title_A, r_label = 1.3)
pB_AIC <- make_radar_models_by_metric(panelB_long, c("linear","exponential"), "AIC", title_B, r_label = 1.3)
pC_AIC <- make_radar_models_by_metric(panelC_long, c("linear","poly2","poly3"), "AIC", title_C, r_label = 1.3)

# Row 2 (R2 only) — models as lines
pA_R2  <- make_radar_models_by_metric(panelA_long, c("linear","exponential"), "R2",  title_A, r_label = 1.3)
pB_R2  <- make_radar_models_by_metric(panelB_long, c("linear","exponential"), "R2",  title_B, r_label = 1.3)
pC_R2  <- make_radar_models_by_metric(panelC_long, c("linear","poly2","poly3"), "R2", title_C, r_label = 1.3)
...
# =========================
# Build a full shared legend that includes all models
# =========================
# Dummy with all four models present so legend shows linear, exponential, poly2, poly3
dummy_df <- data.frame(
...
# =========================
# Arrange 2 rows x 3 columns, legend on top
# =========================
...
label_R2 <- ggplot() +
  annotate("text", x = 0.5, y = 0.5,
           label = "R^2", parse = TRUE,         # proper superscript 2
           angle = 0, size = 9) +
  label_theme
