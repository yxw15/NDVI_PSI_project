...
for (i in seq_along(dist_seq)) {
  ...
  if (sum(unlist(nb)) == 0) {
    ...
    test <- moran.test(Quantiles, lw, zero.policy = TRUE)
  ...
  # Save the individual result as a CSV file with the naming format "Quantiles_spatial_[distance].csv"
  file_name <- sprintf("results_spatial/Quantiles_spatial_%.1f.csv", d)
