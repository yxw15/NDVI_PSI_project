...
# ---- TD (transpiration deficit) binner: Germany vs Species ----
TD_bin <- function(df, bin_width = 3, min_count = 1000) {
  ...
  td_min <- floor(min(df$transpiration_deficit, na.rm = TRUE))
  td_max <- ceiling(max(df$transpiration_deficit, na.rm = TRUE))
  bin_breaks <- seq(td_min, td_max, by = bin_width)
  ...
  df %>%
    ...
    select(area, species, TD_bin, bin_mean, avg_value, count, total_pixels, percentage)
}

TD_diff_species_minus_Germany_bin <- function(df, bin_width = 3, min_count = 1000) {
  ...
  sp_tbl %>%
    inner_join(de_tbl, by = c("species", "TD_bin")) %>%
    mutate(
      diff_NDVI = sp_mean - de_mean,
      pct_bin   = pmin(sp_pct, de_pct)
    ) %>%
    select(species, TD_bin, bin_mean, sp_mean, de_mean, sp_n, de_n, sp_pct, de_pct, pct_bin, diff_NDVI)
}

# ---- Plot: TD bins + bar difference (Germany vs species) ----

plot_F4_NDVI_TDbin_area_Bar_Diff <- function(
    df,
    out_file = "results_rootzone/Figures_till2022/SI_PSI/SI_NDVI_TDbin_area_Bar_Diff.png",
    bin_width = 3,              # <-- 3 mm bins
    min_count = 1000,
    k_left = 3,
    gam_method = "REML",
    width = 12,
  ...
  d_all <- TD_bin(df, bin_width = bin_width, min_count = min_count) %>%
  ...
  p_left <- ggplot(d_sp, aes(bin_mean, avg_value)) +
    ...
    labs(x = "transpiration deficit (mm)", y = "NDVI quantiles (rank)", title = "(a)") +
    base_theme +
    guides(
      size = guide_legend(
        order = 1,
  ...
  # --- Bar Logic ---
  df_diff <- TD_diff_species_minus_Germany_bin(df, bin_width, min_count) %>%
    mutate(species = factor(species, levels = sp_levels))
...
# ---- Run ----
plot_F4_NDVI_TDbin_area_Bar_Diff(
  df = DT_combined,
  out_file = "results_rootzone/Figures_till2022/SI_PSI/SI_NDVI_TDbin_area_Bar_Diff.png",
  bin_width = 3,
  min_count = 1000
